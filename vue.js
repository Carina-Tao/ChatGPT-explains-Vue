...
/**
 * 这段代码是一个 UMD (Universal Module Definition) 的模块化规范的实现。
 * UMD 是一种兼容多种模块化规范的方案，可以让你的代码在不同的环境中都能够正确的运行。
 * 这个模块化规范的实现首先判断当前环境是否支持 CommonJS 规范（即 exports 对象是否存在），如果存在则将模块作为一个 CommonJS 模块输出。
 * 如果当前环境支持 AMD 规范（即 define 函数是否存在），则将模块作为一个 AMD 模块输出。
 * 如果都不支持，则将模块挂载在全局对象上，即 window 对象或 global 对象上，并赋值给 Vue。
 * 这段代码中的 factory 函数即为 Vue 的构造函数，它会根据不同的模块化规范来进行输出。
 * 在 CommonJS 环境中，通过 module.exports 输出；
 * 在 AMD 环境中，通过 define 函数输出；在浏览器环境中，则通过全局对象挂载。
 */
;(function (global, factory) {
  ...
  /**
   * 这段代码创建了一个空的对象，并且使用 Object.freeze 方法将其冻结，从而防止该对象被修改。
   * 因为对象是引用类型，如果一个对象没有被冻结，那么可以通过改变其属性来修改该对象，即使它被声明为 const。
   * 通过冻结对象，可以保证其属性不会被修改，从而增强代码的可靠性。
   */
